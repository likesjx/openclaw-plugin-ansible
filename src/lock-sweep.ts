...
function extractPid(lockBody: string): number | null {
...
async function* walk(dir: string, maxDepth: number): AsyncGenerator<string> {
...
async function sweepLocks(params: {
  ...
  rootDir: string;
  staleSeconds: number;
}): Promise<{ removed: number; kept: number; errors: number; found: number }> {
  ...
  for await (const filePath of walk(agentsDir, 4)) {
    if (!filePath.endsWith(".jsonl.lock")) continue;
    found++;
...
export function createLockSweepService(
