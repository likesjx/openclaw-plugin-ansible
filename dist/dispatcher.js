...
export function startMessageDispatcher(api, config) {
    ...
    api.logger?.info(`Ansible dispatcher: watching messages map (${seen.size} existing messages skipped)`);
    // Observe new entries
    messages.observe((event) => {
        for (const [id, change] of event.changes.keys) {
            if (change.action !== "add")
                continue; // only new messages
            if (seen.has(id))
                continue;
            seen.add(id);
            ...
            if (Array.isArray(msg.readBy) && msg.readBy.includes(myId))
                continue;
            api.logger?.info(`Ansible dispatcher: new message ${id.slice(0, 8)} from ${msg.from}`);
    ...
    try {
        ...
        await reply.dispatchReplyWithBufferedBlockDispatcher({
            ...
            dispatcherOptions: {
                ...
                onError: (err, info) => {
                    api.logger?.warn(`Ansible dispatcher: ${info.kind} reply error: ${String(err)}`);
                },
            },
        });
        ...
        if (doc) {
            const messagesMap = doc.getMap("messages");
            const current = messagesMap.get(messageId);
        ...
        api.logger?.info(`Ansible dispatcher: finished processing message ${messageId.slice(0, 8)}`);
    }
    catch (err) {
        api.logger?.warn(`Ansible dispatcher: failed to dispatch message ${messageId.slice(0, 8)}: ${String(err)}`);
    }
}
