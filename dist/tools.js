...
function writeSendReceiptMessage(doc, fromNodeId, senderAgentId, originalMessageId, originalTargets, originalContent, notifyToAgents) {
    ...
    const content = `[Ansible Send Receipt] ${senderAgentId} posted on ansible.\n` +
...
function executeCapabilityPublishPipeline(args) {
    ...
    const normalized = (ids) => Array.from(new Set(ids)).sort();
    ...
    const priorStage = installStageMap.get(manifestKey);
    let installTransition;
    if (priorStage &&
    ...
    const stageAfterInstall = installStageMap.get(manifestKey);
    ...
    const priorWiring = wiringMap.get(manifestKey);
    let wireTransition;
    if (priorWiring &&
...
export function registerAnsibleTools(api, config) {
    ...
    api.registerTool({
        ...
        description: "Admin-only: fail active skill-distribution tasks in bulk (incident stop-lever).",
        parameters: {
            type: "object",
            properties: {
                dry_run: {
        ...
        async execute(_id, params) {
            ...
            try {
                ...
                const limit = typeof params.limit === "number" && Number.isFinite(params.limit)
                    ...
                    : 5_000;
                const reasonSuffix = typeof params.reason === "string" && params.reason.trim().length > 0
                ...
                for (const [taskKey, raw] of tasks.entries()) {
                    ...
                    scanned += 1;
                    const task = raw;
                    if (!task || task.intent !== "skill_distribution")
                        continue;
                    if (task.status !== "pending" && task.status !== "claimed" && task.status !== "in_progress")
                        continue;
                    candidates.push({
                ...
                if (!dryRun) {
                    ...
                    for (const c of candidates) {
                        const key = resolveTaskKey(tasks, c.taskId);
                        if (typeof key !== "string")
                            continue;
                        const task = tasks.get(key);
                        if (!task)
    ...
    api.registerTool({
        ...
        description: "Return install/wire lifecycle state for a capability version to support idempotency and rollout evidence capture.",
        parameters: {
            type: "object",
            properties: {
                ...
                version: {
                    type: "string",
                    description: "Optional explicit version. If omitted, uses current active version, otherwise most recent catalog version.",
                },
            },
            required: ["capability_id"],
        },
        async execute(_id, params) {
            ...
            try {
                requireAuth(nodeId);
                const capabilityId = validateString(params.capability_id, 160, "capability_id").trim();
                const requestedVersion = typeof params.version === "string" && params.version.trim().length > 0
                ...
                const resolvedVersion = requestedVersion || revision?.activeVersion || catalog?.version || revision?.versions?.[revision.versions.length - 1];
                if (!resolvedVersion) {
                ...
                const installStage = installStageMap.get(manifestKey);
                const wiring = wiringMap.get(manifestKey);
                const normalized = (ids) => Array.from(new Set((ids || []).map(String))).sort();
    ...
    api.registerTool({
        ...
        async execute(_id, params) {
            ...
            try {
                ...
                const routedExecutorSkills = Array.from(new Set(capabilityContracts
                    .map((c) => String(c.executorSkillRef?.name || "").trim())
                    .filter((name) => name.length > 0)));
                const routedDelegationSkills = Array.from(new Set(capabilityContracts
    ...
    api.registerTool({
        ...
        parameters: {
            ...
            properties: {
                ...
                notify_to: {
                    type: "string",
                    description: "Optional receipt target agent id (or comma-separated list). Overrides default gateway-admin receipt routing.",
                },
                metadata: {
                    type: "object",
                    description: "Optional structured metadata (e.g., CoreMetadata fields like conversation_id, corr, kind).",
